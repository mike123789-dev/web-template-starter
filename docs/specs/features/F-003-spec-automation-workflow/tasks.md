...
| T-002 | task 상세 문서(T-002)를 task-governance 기준으로 구체화 | FR-002, NFR-003 | 상세 task 실행 기준 문서 | `npm run specs:check` | `tasks/T-002.md` | Done |
| T-003 | AC-테스트 매핑 및 진행판 상태(`Blocked/Clarification`) 정상화 | FR-005, NFR-003 | 매핑 누락 0건, blocked 0건 | `npm run specs:check` | `-` | Done |
